      ...
      {emulators.length > 0 && (
        <div className="flex flex-col gap-1.5">
          {emulators.map((emulator) => (
            <div
              key={emulator.id}
              className="flex items-center gap-2.5 rounded-[9px] border border-border bg-white/[0.02] px-2.75 py-2"
            >
        ...
        <div>
          ...
          <div className="flex gap-1.5">
            <input
              ...
              className={`${textInputClass} min-w-0 flex-1 font-mono text-[11.5px]`}
            />
            <button
              type="button"
              onClick={handleBrowse}
              title="Browse for the executable"
              className="flex flex-none items-center justify-center rounded-[9px] border border-input bg-white/[0.03] px-3 text-muted-foreground hover:text-foreground"
            >
              <FolderOpen size={15} />
            </button>
          </div>
