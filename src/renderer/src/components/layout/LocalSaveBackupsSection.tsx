...
export const LocalSaveBackupsSection = (): React.JSX.Element => {
  ...
  // mismo lenguaje que Images: gris para lo que un clic se puede llevar.
  const keptBytes = Math.max(0, (usage?.totalBytes ?? 0) - reclaimableBytes);
  const segments = usage
    ? [
        { key: 'kept', bytes: keptBytes, color: TEAL },
        { key: 'reclaimable', bytes: reclaimableBytes, color: GRAY },
  ...
  return (
    <SettingsCard
      layout="column"
      title="Save backups"
      description="A local working copy of your cloud saves — the actual restore always comes from the cloud, never from here."
      icon={Archive}
      color={TEAL}
          ...
          <div className={BAR_CLASS}>
            {segments.map((segment) => (
              <div
                ...
                style={{
                  width: `${Math.max(2, (segment.bytes / (usage?.totalBytes ?? 1)) * 100)}%`,
